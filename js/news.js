...
async function loadPatchNotes() {
    ...
    try {
        ...
        if (res.status === 429) {
            grid.innerHTML = "<p style='color:var(--text);'>Too many requests. Try again later.</p>";
            return;
        }
        ...
        const patches = Array.isArray(data) ? data : (data.formatted_patches || data.patch_notes || data.patchNotes || data.data || []);
